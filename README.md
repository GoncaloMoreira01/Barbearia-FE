💈 Barbershop-FE  **(Work in Progress)**
===============

Frontend web application for managing **barbershop appointments**, built with **Angular**. The platform allows customers to schedule services, view appointment history, and leave feedback, while barbers can manage their schedules and see upcoming bookings.

This project demonstrates **role-based access control, reactive programming with Observables, and modern UI development with Angular Material**.

🚀 Features
===========

### 🔐 Authentication

*   User login system
    
*   Role-based access (Client / Barber)
    
*   Secure route protection
    

### 📅 Appointment Management

*   Schedule new appointments
    
*   View **upcoming appointments**  **(coming soon)**
    
*   View **past appointment history**   **(coming soon)**
    
*   Manage personal bookings **(coming soon)**
    

### 👤 Client Features

*   Create and manage appointments **(manage coming soon)**
    
*   View previous services  **(coming soon)**
    
*   Leave feedback after appointments
    

### ✂️ Barber Features

*   View scheduled appointments
    
*   Manage upcoming bookings
    
    

### ⭐ Feedback System

*   Clients can rate their experience
    
    

🛠️ Technologies Used
=====================

*   **Angular**
    
*   **TypeScript**
    
*   **HTML5 / CSS3**

*   **Bootstrap**
    
*   **Angular Material UI**

*   **Angular CLI**
    
*   **Angular Router**
    
*   **Reactive Reactive Forms**

*   **Angular HttpClient**
    
*   **RxJS Observables**
    
*   **REST API Integration**

⚠️ Project Status
=====================
This project is currently under active development. 
New features such as appointment history and appointment management are being implemented.